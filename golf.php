...
class PwgPlugin
{
    ...
    function register(){
        ...
        require_once( PWG_PLUGIN_DIR . '/classes/golfClub.php' );
        ...
        add_action( 'show_user_profile', [$this, 'add_golfclub_field'] );
        add_action( 'edit_user_profile', [$this, 'add_golfclub_field'] );

        add_action( 'personal_options_update', [$this, 'save_golfclub_field'] );
        add_action( 'edit_user_profile_update', [$this, 'save_golfclub_field'] );
        ...
        add_action('wp_ajax_pwg_add_new_golfclub', [golfClub::class, 'add_new_golfclub']);
        add_action('wp_ajax_nopriv_pwg_add_new_golfclub', [olfClub::class, 'add_new_golfclub']);

        add_action('wp_ajax_update_club', [new golfClub(), 'update_club']);
        add_action('wp_ajax_nopriv_update_club', [new golfClub(), 'update_club']);
    ...
    function addNewClub(){
        ...
        if ( isset( $_POST['clubname'] ) ){
            ...
            $newClub = new golfClub(0);

            $newClub->setClubName( sanitize_text_field( $_POST['clubname'] ) );
            $newClub->setAboutClub( sanitize_text_field( $_POST['aboutclub'] ) );
            $newClub->setAddress1( sanitize_text_field( $_POST['address_1'] ) );
            ...
            $newClub->setPhone( sanitize_text_field( $_POST['phone'] ) );
    ...
    function add_golfclub_field( $user ) {
        ...
        if($clubs) {
          ...
          echo '<tr>';
          echo '<th><label for="golfclub">Your Home Club</label></th>';
      
          echo '<td>';
          echo '<select name="golfclub" id="golfclub">';
          
          foreach ( $clubs as $club ) {
            $selected = get_the_author_meta( 'golfclub', $user->ID ) == $club->ID ? 'selected' : '';
      ...
      function save_golfclub_field( $user_id ) {
    ...
    function getClub($content){
        if (is_singular('golfclub') && in_the_loop() && is_main_query()) {
            ...
            $golf_club = new GolfClub($club_ID);

            //$content_to_add = $golf_club->showClub();
            $content_to_add = $golf_club->show_course_scorecard();
            if ($content_to_add) {
    ...
    function getCourse($content){
        if (is_singular('golfcourse') && in_the_loop() && is_main_query()) {
            ...
            $golf_course = new GolfCourse($course_ID);
...
<form id="radioform">
	<table>
		<tbody>
		<tr>
			<td><input class="pref" checked="checked" id="book" name="book" type="radio" value="Sycamore Row" />Sycamore Row</td>
			<td>John Grisham</td>
		</tr>
		<tr>
			<td><input class="pref" id="book" name="book" type="radio" value="Dark Witch" />Dark Witch</td>
			<td>Nora Roberts</td>
		</tr>
		</tbody>
	</table>
